...
----
-- Check placement of non-sort output expressions around Sort without LIMIT.
----
...
-- If a post-sort projection is already needed, postpone other non-sort
-- expressions that need no extra Sort input.
EXPLAIN (VERBOSE, COSTS OFF)
SELECT random(), repeat(g.i::text, 100)
  FROM generate_series(1, 100) g(i)
  ORDER BY g.i;
