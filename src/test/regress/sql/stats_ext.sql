...
-- outer-join nullingrels should not prevent matching expression dependencies
CREATE STATISTICS mcv_lists_coalesce_deps (dependencies)
  ON (coalesce(mod(a,20),1)), (coalesce(mod(b::int,10),1)), (coalesce(mod(c,5),1))
  FROM mcv_lists;
...
DROP STATISTICS mcv_lists_coalesce_deps;
