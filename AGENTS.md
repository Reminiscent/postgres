...
4. Before every commit, run the formatting workflow in `LOCAL_WORKFLOWS.md` from the worktree root. If `pg_bsd_indent` is missing for that worktree, build it from `src/tools/pg_bsd_indent`, then run `src/tools/pgindent/pgindent .` and review the resulting diff as part of the change.
5. Before every commit or commit-message rewrite, write a message that
   describes the current diff accurately: what changed, why it is
   needed, and any user-visible behavior or compatibility impact that
   follows from this patch. Keep every line at or under 72 characters.
   Do not add attribution, discussion links, backpatch scope, release
   notes, or other committer-facing metadata unless the task evidence
   already supports it or the user explicitly asks for it.
6. When an agent run needs a local cluster for tests or validation, that same run must start the worktree-local cluster explicitly and stop it before handoff. Do not rely on a pre-started server or leave an agent-started cluster running.
7. Treat concurrency, visibility, locking, and transaction semantics as isolation-test candidates by default.
8. If docs, client behavior, output files, catalog state, compatibility, or upgrade behavior may change, call that out explicitly.
9. A task is not done without exact commands or exact evidence references, plus concise result summaries.
...
11. For non-trivial design-sensitive `change` or `investigate` work, use `pg-design-research` when external system precedent or relevant literature would materially affect the design choice. PostgreSQL local sources remain authoritative.
12. Treat PostgreSQL mailing lists as first-class project evidence: `pgsql-hackers` for design and patch discussion, `pgsql-bugs` for bug reports and reproductions, `pgsql-performance` for workload and regression evidence, `pgsql-committers` for landed outcomes and backpatch signals.
