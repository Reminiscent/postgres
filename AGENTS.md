...
5. When an agent run needs a local cluster for tests or validation, that same run must start the worktree-local cluster explicitly and stop it before handoff. Do not rely on a pre-started server or leave an agent-started cluster running.
6. Treat concurrency, visibility, locking, and transaction semantics as isolation-test candidates by default.
7. If docs, client behavior, output files, catalog state, compatibility, or upgrade behavior may change, call that out explicitly.
8. A task is not done without exact commands or exact evidence references, plus concise result summaries.
...
10. For non-trivial design-sensitive `change` or `investigate` work, use `pg-design-research` when external system precedent or relevant literature would materially affect the design choice. PostgreSQL local sources remain authoritative.
11. Treat PostgreSQL mailing lists as first-class project evidence: `pgsql-hackers` for design and patch discussion, `pgsql-bugs` for bug reports and reproductions, `pgsql-performance` for workload and regression evidence, `pgsql-committers` for landed outcomes and backpatch signals.
